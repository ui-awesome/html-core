...
- feat: add automatic `aria-describedby` support to `BaseInput`.
...
- fix: accept string `true` in the `aria-describedby` condition for `BaseInput`.
...
- feat: add `ariaDescribedBySuffix` configuration to `BaseInput`.
- fix: correct `ariaDescribedBySuffix` formatting and related input tests.
